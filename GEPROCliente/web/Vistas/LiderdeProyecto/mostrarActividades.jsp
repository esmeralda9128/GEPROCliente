<%-- 
    ...
    Created on : 08-abr-2019, 17:10:12
    Author     : Alberto
--%>
<%
...
<html>
    ...
    <body>
        ...
        <div class="offset-md-2 container">
            <br/>
            <h1 style="float:left">Actividades Realizadas</h1>
        ...
        <script>
            ...
            peticion.onreadystatechange = function () {
                if (this.status === 200 && this.readyState === 4) {
                    ...
                    var arr = respuesta.respuesta.actividades;
                }
                $('#tablaActividades').html('');
                if (arr.length !== 0) {
                    for (var i = 0; i < arr.length; i++) {
                        $('#tablaActividades').append('<tr><td>' + arr[i].actividad + '</td><td>' + arr[i].descripcion + '</td><td>' + arr[i].fechaActividad + '</td></tr>');
                    }
                } else {
                    Swal.fire({
                        title: 'Aviso',
                        text: "Aún no hay actividades realizadas",
                        type: 'info',
                        confirmButtonColor: '#009475',
                        confirmButtonText: 'Aceptar'
                    })
                    $('#tablaActividades').append('<h2>No hay actividades registradas.</h2>');
                }
            };
            peticion.open("GET", "http://localhost:8080/GEPROServidor/servicioGEPRO/proyecto/mostrarActividades");
            peticion.send();
        </script>
